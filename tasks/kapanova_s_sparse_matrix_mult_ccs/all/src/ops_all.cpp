...
std::vector<size_t> ComputeBalancedRanges(int total_cols, int num_procs, const CCSMatrix &b) {
  std::vector<size_t> ranges(num_procs + 1);
  ranges[0] = 0;
  ranges[num_procs] = static_cast<size_t>(total_cols);
  int total_nnz = b.col_ptrs[total_cols];
  int nnz_per_proc = total_nnz / num_procs;
  int current_col = 0;
  for (int p = 1; p < num_procs; ++p) {
    int target_nnz = p * nnz_per_proc;
    while (current_col < total_cols && static_cast<int>(b.col_ptrs[current_col]) < target_nnz) {
      ++current_col;
    }
...
bool KapanovaSSparseMatrixMultCCSALL::RunImpl() {
  ...
  if (mpi_rank == 0) {
    auto raw = ComputeBalancedRanges(c.cols, mpi_size, b);
...
#pragma omp parallel for schedule(guided, 32)
  ...
  std::vector<uint64_t> flat_rows;
  std::vector<double> flat_vals;
  size_t total_local_nnz = 0;
  for (size_t j = 0; j < local_cols; ++j) {
    total_local_nnz += temp_rows[j].size();
  }
  flat_rows.reserve(total_local_nnz);
  flat_vals.reserve(total_local_nnz);
  for (size_t j = 0; j < local_cols; ++j) {
    for (size_t v : temp_rows[j]) {
      flat_rows.push_back(static_cast<uint64_t>(v));
    }
